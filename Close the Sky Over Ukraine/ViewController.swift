...
//    }
    ...
    @objc func keyboardWillHide(_ notification: NSNotification) {
        ClosetheSkyOverUkraine.webView.setNeedsLayout()
    }
    
    func initWebView() {
        ClosetheSkyOverUkraine.webView = createWebView(container: webviewView, WKSMH: self, WKND: self, NSO: self, VC: self)
        webviewView.addSubview(ClosetheSkyOverUkraine.webView);
        ClosetheSkyOverUkraine.webView.uiDelegate = self;
        ClosetheSkyOverUkraine.webView.addObserver(self, forKeyPath: #keyPath(WKWebView.estimatedProgress), options: .new, context: nil)
...
//        toolbarView.autoresizingMask = [.flexibleTopMargin, .flexibleRightMargin, .flexibleWidth]
    ...
    @objc func loadRootUrl() {
        ...
        ClosetheSkyOverUkraine.webView.load(URLRequest(url: launchUrl))
    }
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!){
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.8) {
            ClosetheSkyOverUkraine.webView.isHidden = false;
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        ...
        if (keyPath == #keyPath(WKWebView.estimatedProgress) &&
                ClosetheSkyOverUkraine.webView.isLoading &&
                !self.loadingView.isHidden &&
                !self.htmlIsLoaded) {
                    var progress = Float(ClosetheSkyOverUkraine.webView.estimatedProgress);
    ...
    deinit {
        ClosetheSkyOverUkraine.webView.removeObserver(self, forKeyPath: #keyPath(WKWebView.estimatedProgress))
    }
}
...
extension ViewController: WKScriptMessageHandler {
  func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        if message.name == "print" {
            printView(webView: ClosetheSkyOverUkraine.webView)
        }
